...
abstract class GenerateTypeSafeCatalogTask : DefaultTask() {
    ...
    fun generate() {
        ...
        catalog.libraryAliases.forEach { alias ->
            val property = PropertySpec.builder(alias.toCamelCase(), ParameterizedTypeName.get(
                ClassName("org.gradle.api.provider", "Provider"),
                ClassName("org.gradle.api.artifacts", "MinimalExternalModuleDependency")
            ))
                .getter(FunSpec.getterBuilder()
        ...
        catalog.bundleAliases.forEach { alias ->
            val property = PropertySpec.builder(alias.toCamelCase(), ParameterizedTypeName.get(
                ClassName("org.gradle.api.provider", "Provider"),
                ClassName("org.gradle.api.artifacts", "ExternalModuleDependencyBundle")
            ))
                .getter(FunSpec.getterBuilder()
        ...
        catalog.pluginAliases.forEach { alias ->
            val property = PropertySpec.builder(alias.toCamelCase(), ParameterizedTypeName.get(
                ClassName("org.gradle.api.provider", "Provider"),
                ClassName("org.gradle.plugin.use", "PluginDependency")
            ))
                .getter(FunSpec.getterBuilder()
